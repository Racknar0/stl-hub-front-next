...
export default function ModalSyncMultimodalVectors({ open, onClose }) {
    ...
    useEffect(() => {
        if (open) {
            fetchStatus();
            setLogs([]);
            setLimit(10); // Start with 10 for testing
            setSyncing(false);
        }
    }, [open]);
    ...
    const fetchStatus = async () => {
        setLoadingInfo(true);
        try {
            const res = await http.getData('/ai/sync-multimodal-status');
            setStatus(res.data);
        } catch (error) {
            console.error('Error fetching multimodal vector status:', error);
            setStatus(null);
        } finally {
    ...
    const handleSync = () => {
        ...
        setSyncing(true);
        setLogs(prev => [...prev, '[INFO] Iniciando conexión SSE para Multimodal...']);

        const apiUrl = (configuredBaseUrl || 'http://localhost:3001')
            .replace(/\/api\/?$/, '')
            .replace(/\/$/, '');
        const url = `${apiUrl}/api/ai/sync-multimodal-missing`;
    ...
    return (
        <Dialog open={open} onClose={!syncing ? onClose : null} fullWidth maxWidth="sm">
            <DialogTitle sx={{ fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: 1 }}>
                <AutoAwesomeIcon color="secondary" /> Sincronizar Vectores Multimodal (IA)
            </DialogTitle>
                ...
                {loadingInfo ? (
                    ...
                    <Box mb={3}>
                        <Typography variant="body2" color="text.secondary" gutterBottom>
                            Esta opción generará los embeddings combinando TEXTO e IMÁGENES usando Gemini Embed 2 en la colección stls-multimodal.
                        </Typography>

                        <Paper variant="outlined" sx={{ p: 2, mt: 2, mb: 2, bgcolor: 'background.default' }}>
                            <Box display="flex" justifyContent="space-between" mb={1}>
                                <Typography variant="subtitle2">Assets Activos (Total):</Typography>
                                <Typography variant="subtitle2">{status?.dbCount || 0}</Typography>
                            </Box>
                            <Box display="flex" justifyContent="space-between" mb={1}>
                                <Typography variant="subtitle2">Vectores Multimodales en Qdrant:</Typography>
                        ...
                        <Box display="flex" alignItems="center" gap={2}>
                            ...
                            <Typography variant="caption" color="text.secondary">
                                Comienza probando con 10 para verificar el proceso.
                            </Typography>
                        </Box>
                    </Box>
                )}
