...
export async function generateAssetMetadata(slug, isEn) {
    ...
    if (isAdult) {
        return {
            ...
            alternates: {
                canonical: `${site}/asset/${slug}`,
                languages: {
                    'es-ES': `${site}/asset/${slug}`,
                    'en-US': `${site}/en/asset/${slug}`,
                    'x-default': `${site}/asset/${slug}`,
                },
            },
            openGraph: {
                title: baseTitle,
...
export default async function AssetPageContent({ slug, isEn }) {
    ...
    const displayTitle = isEn ? (asset.titleEn || asset.title || 'STL Model') : (asset.title || 'Modelo STL');
    const displayDesc = isEn ? descriptionEn : descriptionEs;
    const premiumLabel = isEn ? 'Premium' : 'Premium';
        ...
        '@graph': [
            {
                ...
                keywords: (isEn ? asset.tagsEn : asset.tagsEs || []).join(', '),
            ...
            },
            {
                ...
                itemListElement: [
                    ...
                    },
                    {
                        '@type': 'ListItem',
                        position: 2,
                        name: isEn ? (categories[0]?.en || 'Catalog') : (categories[0]?.es || 'Catálogo'),
                        item: isEn ? `${site}/en/search${categories[0]?.slug ? `?category=${categories[0].slug}` : ''}` : `${site}/search${categories[0]?.slug ? `?category=${categories[0].slug}` : ''}`
                    },
                    {
                        '@type': 'ListItem',
                        position: 3,
                        name: displayTitle,
                        item: isEn ? `${site}/en/asset/${asset.slug}` : `${site}/asset/${asset.slug}`
                    }
                ]
            }
        ],
    ...
    return (
        <>
            ...
            <NsfwPageWrapper isAdult={isAdult} isEn={isEn}>
                <main className={styles.page}>
                <div className={styles.backBar}>
                    <Link href={isEn ? "/en" : "/"} className={styles.backBtn}>
                    ...
                    <section className={styles.grid}>
                        <article className={styles.card}>
                            <h2>
                                ...
                                {isEn ? `Technical Sheet of ${displayTitle}` : `Ficha técnica de ${displayTitle}`}
                            </h2>
                            <dl className={styles.metaList}>
                                <div><dt>ID</dt><dd>{asset.id || 'N/A'}</dd></div>
                                <div><dt>{isEn ? 'Type' : 'Tipo'}</dt><dd>{asset.isPremium ? (isEn ? 'Premium content' : 'Contenido premium') : (isEn ? 'Free content' : 'Contenido gratuito')}</dd></div>
                                <div><dt>{isEn ? 'Published' : 'Publicado'}</dt><dd>{formatDate(asset.createdAt)}</dd></div>
                                <div><dt>{isEn ? 'Updated' : 'Actualizado'}</dt><dd>{formatDate(asset.updatedAt)}</dd></div>
                                <div><dt>{isEn ? 'File size' : 'Tamaño archivo'}</dt><dd>{formatBytes(asset.archiveSizeB || asset.fileSizeB)}</dd></div>
                                <div><dt>{isEn ? 'Category' : 'Categoría principal'}</dt><dd>{isEn ? (categories[0]?.en || categories[0]?.es || 'N/A') : (categories[0]?.es || 'N/A')}</dd></div>
                            </dl>
                        </article>
                        ...
                        <article className={styles.card}>
                            <h2>
                                ...
                                {isEn ? `Categories & Tags for ${displayTitle}` : `Categorías y Tags de ${displayTitle}`}
                            </h2>
                            <p className={styles.metaHint}>{isEn ? 'Thematic labels for navigation and indexing:' : 'Etiquetas temáticas para navegación e indexación:'}</p>
                            <div className={styles.tagWrap}>
                                {mergedTags.length === 0 ? <span className={styles.badgeMuted}>Sin tags</span> : null}
                                {mergedTags.map((t, i) => (
                                    <Link
                                        key={`${t.slug}-${i}`}
                                        href={isEn ? `/en/search?tags=${encodeURIComponent(t.slug)}` : `/search?tags=${encodeURIComponent(t.slug)}`}
                            ...
                            <div className={styles.categoryWrap}>
                                {categories.map((cat, i) => (
                                    <span key={`${cat.slug || cat.es}-${i}`} className={styles.categoryChip}>
                                        {isEn ? (cat.en || cat.es) : cat.es}{cat.en && cat.en !== cat.es ? ` / ${cat.en}` : ''}
                                    </span>
                                ))}
                            </div>
                        </article>
                ...
                {displayDesc && (
                    <div className={styles.contentWrap}>
                        <section className={styles.descriptionCard}>
                            <h2>{isEn ? `Description of ${displayTitle}` : `Descripción de ${displayTitle}`}</h2>
                            <p>{displayDesc}</p>
                        </section>
                    </div>
                )}
                    ...
                    <div className={styles.contentWrap}>
                        <section className={styles.galleryCard}>
                            <h2>
                                ...
                                {isEn ? `Image Gallery for ${displayTitle}` : `Galería de imágenes de ${displayTitle} para impresión 3D`}
                            </h2>
